...
| `LOG_FORMAT` | Morgan-style log format: `tiny`, `short`, `dev`, `debug`, `common`, or `combined`. `debug` appends a colorized, indented JSON `ctx:` block from `RequestContext.data` (including authenticated user fields like `authUser`). | `dev` |
